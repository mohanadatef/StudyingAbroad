        ...
        {{csrf_field()}}
            <div class="form-group{{ $errors->has('name') ? ' has-error' : "" }}">
               <input type="text" value="{{Request::old('name')}}" class="form-control find-us-input"
                                    name="name" placeholder="{{ trans('lang.Name') }}">
            </div>
            <div class="form-group{{ $errors->has('email') ? ' has-error' : "" }}">
                <input type="text" value="{{Request::old('email')}}" class="form-control find-us-input"
                       name="email" placeholder="{{ trans('lang.Email') }}">
            </div>
            <div class="form-group{{ $errors->has('message') ? ' has-error' : "" }}">
                <textarea  id="exampleFormControlTextarea1" value="{{Request::old('message')}}"
                          placeholder="{{ trans('lang.Message') }}" class="form-control  find-us-area" name="message" ></textarea>
            </div>
            <button type="submit" class="btn btn-primary contact-send-btn">  <i class="fas fa-paper-plane"></i>{{ trans('lang.SEND') }}</button>
    </form>
</div>
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h2 class="book-modal-title "><span class="book-modal-title-span">{{ trans('lang.Success') }}</span></h2>
                ...
                <div class="modal-footer" style="justify-content: center;border: none;">
                    ...
                    <button type="button"  class="btn btn-primary"
                            style="min-width: 100px;background: #00adef;border:none;" data-dismiss="modal">{{ trans('lang.OK') }}</button>
                </div>
            </div>
        </div>
    </div>
